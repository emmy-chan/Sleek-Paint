...
namespace ImGui
{
    ...
    public:
        ...
        unsigned int GetTypeFilterIndex() const noexcept;
...
inline void ImGui::FileBrowser::Display()
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, { 0.5f, 0.3f });
    ...
    SameLine();
    ImGui::SetCursorPosX(ImGui::GetWindowWidth() - 167);

    if (!(flags_ & ImGuiFileBrowserFlags_SelectDirectory))
    {
        if (Button("Ok", {75, 24}) && !selectedFilenames_.empty())
        {
            ok_ = true;
            CloseCurrentPopup();
        }
    ...
    bool shouldExit =
        Button("Cancel", { 75, 24 }) || closeFlag_ ||
    ...
    if (shouldExit)
    {
        //Let's make our life easy and customize this a bit, - emmy :3
...
inline unsigned int ImGui::FileBrowser::GetTypeFilterIndex() const noexcept
{
    return typeFilterIndex_;
}
    ...
    const std::vector<std::string>& _typeFilters)
{
    ...
    //wtf why is this here??? it breaks the combo box
    //typeFilterIndex_ = 0;
}
...
{
    fileRecords_ = { FileRecord{ true, "..", ICON_FA_FOLDER_OPEN"  ..", "" } };
    ...
    for (auto& p : std::filesystem::directory_iterator(pwd_))
    {
        ...
        rcd.showName = (rcd.isDir ? ICON_FA_FOLDER" " : "") +
